...
function eventIsPostponed(event) {
  const status = normalizedStatus(event);
  const flag = String(event.strPostponed || "")
    .trim()
    .toUpperCase();
...
async function fetchEventsForDate(dateString) {
  ...
  const response = await fetch(url, {
    headers: {
      "User-Agent": "NB1-TippLiga-Sync/5.0"
    }
  });
...
function buildMappedEvent(event) {
  return {
    ...
    postponedWithoutDate: eventIsPostponed(event),
...
function isRoundComplete(
  ...
  return roundEvents.every(event => {
    if (
    ...
    return !!existing?.postponedWithoutDate;
  });
}
...
function existingStartDate(existing) {
  const millis = timestampMillis(
    existing?.startTime
  );
...
function buildDesiredData(
  ...
  const apiStart =
    event.startDate?.getTime?.() ?? null;
  const apiHasNewFutureDate = !!(
    locks.postponed &&
    existing?.postponedWithoutDate &&
    !event.postponedWithoutDate &&
    originalStart !== null &&
    apiStart !== null &&
    apiStart > Date.now() &&
    Math.abs(apiStart-originalStart) > 30*60*1000
  );
